...
class Scraper:
    ...
    def userconfigandinstascraper(self):
        try:
            filef = open("ManagerFollower1Conf.txt", "r")
            ...
            self.bottype = "useradmin"
            self.bottype = "follower"
            ...
            self.myinstascraper = InstaScraper(
                "chromedriver.exe", username, password, "Cookies1.txt", 3, displaychrome
            )
    ...
    def scraperfollowings(self):
        try:
            ...
            if random_num <= 14:
                rowcount = self.SqlQueryExec(
                    "SELECT username FROM publicprofiles WHERE Followers >= 100000 and Following != 0 and Following<2000 and Scrapped != 1 order by Rand() Limit 1",
                    True,
                )
            elif random_num > 14 and random_num < 18:
                rowcount = self.SqlQueryExec(
                    "SELECT username FROM publicprofiles WHERE Followers < 100000 and Followers > 25000 and Following != 0 and Following<2000 and Scrapped != 1 order by Rand()  Limit 1",
                    True,
                )
            elif random_num >= 18 and random_num < 20:
                rowcount = self.SqlQueryExec(
                    "SELECT username FROM publicprofiles WHERE Followers <= 25000 and Followers >= 5000 and Following != 0 and Following<2000 & Scrapped != 1 order by Rand()  Limit 1",
                    True,
                )
            else:
                rowcount = self.SqlQueryExec(
                    "SELECT username FROM publicprofiles WHERE Followers < 5000 and Following != 0 and Following<2000 Scrapped != 1 order by Rand() Limit 1",
                    True,
                )
            if rowcount is 0:
                # Error1
